...
export const checkoutSession = onCall<CheckoutSessionData>({ region: 'europe-west1' }, async (request) => {
  const ctx = requireTenantAuth(request, ['admin', 'reception', 'therapist']);
  const data = request.data;

  if (!data?.customerId || !data?.staffId || !data?.roomId || !data.items?.length) {
    throw new HttpsError('invalid-argument', 'Müşteri, terapist, oda ve en az bir hizmet/ürün zorunludur.');
  }
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    if (!staffSnap.exists) throw new HttpsError('not-found', 'Terapist bulunamadı.');
    ...
    for (const id of serviceRefs) {
      if (!servicesById.get(id)) throw new HttpsError('not-found', `Hizmet bulunamadı: ${id}`);
    }
    for (const id of productRefs) {
      if (!productsById.get(id)) throw new HttpsError('not-found', `Ürün bulunamadı: ${id}`);
    }
    ...
    for (const item of data.items) {
      ...
      if (item.kind === 'service') {
        ...
        if (isPackageRedemption) {
          ...
          if (!pkgSnap?.exists || !pkg) throw new HttpsError('not-found', 'Paket bulunamadı.');
          if (pkg.customerId !== data.customerId) throw new HttpsError('failed-precondition', 'Paket bu müşteriye ait değil.');
          if (pkg.status !== 'active') throw new HttpsError('failed-precondition', 'Paket aktif değil.');
          if (pkg.kalanSeans < item.qty) throw new HttpsError('failed-precondition', 'Paketin kalan seans hakkı yetersiz.');
        ...
        if (product.mevcutStok < item.qty) {
          throw new HttpsError('failed-precondition', `"${product.ad}" için stok yetersiz.`);
        }
    ...
    if (Math.abs(paymentsTotal - totalAmount) > EPSILON) {
      throw new HttpsError('failed-precondition', 'Ödeme tutarları toplamı seans tutarına eşit olmalı.');
    }
